...
if (!Number.isFinite(temperature) || temperature < 0 || temperature > 2) {
  throw new Error('AI_TEMPERATURE inválida.');
}
...
export const env = Object.freeze({
  ...
  openRouterApiKey: clean(
    process.env.OPENROUTER_API_KEY
  ),

  knowledgePath: clean(
    process.env.KNOWLEDGE_PATH,
    'knowledge/energy-knowledge.json'
  ),

  structuredTariffPath: clean(
    process.env.STRUCTURED_TARIFF_PATH,
    'knowledge/tariffs-structured.json'
  ),

  knowledgeTopK: Number(
    process.env.KNOWLEDGE_TOP_K ?? 6
  ),

  tariffDefaultDays: Number(
    process.env.TARIFF_DEFAULT_DAYS ?? 30
  ),
